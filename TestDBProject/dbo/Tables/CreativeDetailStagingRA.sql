CREATE TABLE [dbo].[CreativeDetailStagingRA] (
    [CreativeDetailStagingRAID] INT            IDENTITY (1000, 1) NOT NULL,
    [CreativeStgID]             INT            NOT NULL,
    [MediaFormat]               CHAR (10)      NOT NULL,
    [MediaFilePath]             VARCHAR (MAX)  NOT NULL,
    [MediaFileName]             VARCHAR (MAX)  NOT NULL,
    [FileSize]                  BIGINT         NOT NULL,
    [AudioTranscription]        NVARCHAR (MAX) NULL,
    CONSTRAINT [PK_CreativeDetailsRAStg] PRIMARY KEY CLUSTERED ([CreativeDetailStagingRAID] ASC)
);


GO
CREATE NONCLUSTERED INDEX [IX_CreativeDetailStagingRA_CreativeStgId]
    ON [dbo].[CreativeDetailStagingRA]([CreativeStgID] ASC)
    INCLUDE([MediaFormat], [MediaFilePath], [FileSize]);


GO
CREATE TRIGGER [dbo].[CreativeDetailStagingRA_AuditTrail] ON [dbo].[CreativeDetailStagingRA] 
...
SET @TableName = 'CreativeDetailStagingRA'
