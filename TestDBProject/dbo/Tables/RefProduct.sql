CREATE TABLE [dbo].[RefProduct] (
    [RefProductID]        INT          IDENTITY (1, 1) NOT NULL,
    ...
    CONSTRAINT [PK_RefProduct] PRIMARY KEY CLUSTERED ([RefProductID] ASC),
    CONSTRAINT [FK_RefProduct_Advertiser] FOREIGN KEY ([AdvertiserID]) REFERENCES [dbo].[Advertiser] ([AdvertiserID]),
    CONSTRAINT [FK_RefProduct_RefCategory] FOREIGN KEY ([SubCategoryID]) REFERENCES [dbo].[RefSubCategory] ([RefSubCategoryID]),
    CONSTRAINT [FK_RefProduct_RefTargetMarket] FOREIGN KEY ([TargetMarketID]) REFERENCES [dbo].[RefTargetMarket] ([RefTargetMarketID])
);


GO
ALTER TABLE [dbo].[RefProduct] NOCHECK CONSTRAINT [FK_RefProduct_Advertiser];


GO
CREATE NONCLUSTERED INDEX [IX_RefProduct_ProductName]
    ON [dbo].[RefProduct]([ProductName] ASC)
    INCLUDE([RefProductID]);


GO
CREATE NONCLUSTERED INDEX [IX_RefProduct_ProductId]
    ON [dbo].[RefProduct]([RefProductID] ASC)
    INCLUDE([ProductName]);


GO
CREATE TRIGGER [dbo].[RefProduct_AuditTrail] ON [dbo].[RefProduct] 
