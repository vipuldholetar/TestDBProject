CREATE TABLE [dbo].[TVStation] (
    [TVStationID]      INT           IDENTITY (1, 1) NOT NULL,
    [StationShortName] VARCHAR (4)   NOT NULL,
    ...
    CONSTRAINT [PK_TVStationMaster] PRIMARY KEY CLUSTERED ([TVStationID] ASC),
    CONSTRAINT [FK_MarketId] FOREIGN KEY ([MarketID]) REFERENCES [dbo].[Market] ([MarketID])
);


GO
ALTER TABLE [dbo].[TVStation] NOCHECK CONSTRAINT [FK_MarketId];


GO
CREATE TRIGGER [dbo].[TVStation_AuditTrail] ON [dbo].[TVStation] 
