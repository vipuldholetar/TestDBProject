CREATE TABLE [dbo].[OccurrenceCostRA] (
    [OccurrenceCostRAID]        INT          IDENTITY (1, 1) NOT NULL,
    [OccurenceClientFacingRAID] INT          NOT NULL,
    ...
    CONSTRAINT [PK_OCCURRENCESCOSTRA] PRIMARY KEY CLUSTERED ([OccurrenceCostRAID] ASC),
    CONSTRAINT [FK_OCCURRENCESCOSTRA_OCCURRENCECLIENTFACINGRA] FOREIGN KEY ([OccurenceClientFacingRAID]) REFERENCES [dbo].[OccurrenceClientFacingRA] ([OccurrenceClientFacingRAId]),
    CONSTRAINT [FK_OCCURRENCESCOSTRA_RADIODAYPARTS] FOREIGN KEY ([DayPartID]) REFERENCES [dbo].[RadioDayPart] ([RadioDayPartID])
);


GO
CREATE TRIGGER [dbo].[OccurrenceCostRA_AuditTrail] ON [dbo].[OccurrenceCostRA] 
...
SET @TableName = 'OccurrenceCostRA'
