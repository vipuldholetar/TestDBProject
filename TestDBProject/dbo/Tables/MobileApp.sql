CREATE TABLE [dbo].[MobileApp] (
    [MobileAppID]         INT            IDENTITY (1, 2229) NOT NULL,
    ...
    [CreatedDT]           DATETIME       CONSTRAINT [DF_MobileApp_CreateDTM] DEFAULT (getdate()) NOT NULL,
    [CTLegacySeq]         INT            NULL,
    CONSTRAINT [PK_MobileApp] PRIMARY KEY CLUSTERED ([MobileAppID] ASC),
    CONSTRAINT [FK_MobileApp_MediaOutletGenre] FOREIGN KEY ([MediaOutletGenreID]) REFERENCES [dbo].[MediaOutletGenre] ([MediaOutletGenreID])
);


GO
CREATE TRIGGER [dbo].[MobileApp_AuditTrail] ON [dbo].[MobileApp] 
