CREATE TABLE [dbo].[ShippingMethod] (
    ...
    [ShipmentMethodName] VARCHAR (50) NOT NULL,
    [IndNeedTrackingNo]  TINYINT      NULL,
    [IndWeightBasedCost] TINYINT      NULL,
    [CreatedDT]          DATETIME     CONSTRAINT [DF_ShippingMethod_CreateDTM] DEFAULT (getdate()) NOT NULL,
    [CreatedByID]        INT          CONSTRAINT [DF_ShippingMethod_CreateBy] DEFAULT ((1)) NOT NULL,
    [ModifiedDT]         DATETIME     NULL,
    [ModifiedByID]       INT          NULL,
    [MTLegacyCodeID]     INT          NULL,
    CONSTRAINT [PK_ShippingMethod] PRIMARY KEY CLUSTERED ([ShippingMethodID] ASC),
    CONSTRAINT [FK_ShippingMethod_Shipper] FOREIGN KEY ([ShipperID]) REFERENCES [dbo].[Shipper] ([ShipperID])
);


GO
CREATE TRIGGER [dbo].[ShippingMethod_AuditTrail] ON [dbo].[ShippingMethod] 
...
END
GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'Primary Key Shipping method', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'ShippingMethod', @level2type = N'COLUMN', @level2name = N'ShippingMethodID';
