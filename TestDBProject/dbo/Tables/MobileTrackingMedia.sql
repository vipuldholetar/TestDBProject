CREATE TABLE [dbo].[MobileTrackingMedia] (
    [MobileTrackingMediaID] INT      IDENTITY (5122, 1) NOT NULL,
    [WebsiteID]             INT      NULL,
    [MobileAppID]           INT      NULL,
    [TrackStartDate]        DATE     NOT NULL,
    [TrackEndDate]          DATE     NOT NULL,
    [CreateDT]              DATETIME CONSTRAINT [DF_MobileTrackingMedia_CreateDTM] DEFAULT (getdate()) NOT NULL,
    [ModifyDT]              DATETIME NULL,
    [CTLegacySeq]           INT      NULL,
    CONSTRAINT [PK_MobileTrackingMedia] PRIMARY KEY CLUSTERED ([MobileTrackingMediaID] ASC),
    CONSTRAINT [FK_MobileTrackingMedia_MobileApp] FOREIGN KEY ([MobileAppID]) REFERENCES [dbo].[MobileApp] ([MobileAppID]),
    CONSTRAINT [FK_MobileTrackingMedia_Website] FOREIGN KEY ([WebsiteID]) REFERENCES [dbo].[Website] ([WebsiteID])
);


GO
CREATE TRIGGER [dbo].[MobileTrackingMedia_AuditTrail] ON [dbo].[MobileTrackingMedia] 
...
SET @TableName = 'MobileTrackingMedia'
