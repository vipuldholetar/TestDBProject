CREATE TABLE [dbo].[CreativeDetailODR] (
    [CreativeDetailODRID]     INT           IDENTITY (1, 1) NOT NULL,
    ...
    [AdFormatId]              INT           NULL,
    CONSTRAINT [PK_CreativeDetailODR] PRIMARY KEY CLUSTERED ([CreativeDetailODRID] ASC),
    CONSTRAINT [FK_CreativeDetailODR_To_ODRAdFormatMap ] FOREIGN KEY ([AdFormatId]) REFERENCES [dbo].[ODRAdFormatMap] ([MTFormatID])
);


GO
CREATE NONCLUSTERED INDEX [idx_CreativeMasterID]
    ON [dbo].[CreativeDetailODR]([CreativeMasterID] ASC);


GO
CREATE TRIGGER [dbo].[CreativeDetailODR_AuditTrail] ON [dbo].[CreativeDetailODR] 
...
SET @TableName = 'CreativeDetailODR'
