CREATE TABLE [dbo].[TVMMPRCode] (
    [TVMMPRCodeCODE]        VARCHAR (200) NOT NULL,
    [ApprovedMarketID]      INT           NULL,
    [OriginalPatternCode]   VARCHAR (200) NOT NULL,
    ...
    CONSTRAINT [PK_TVMMPRCodes] PRIMARY KEY CLUSTERED ([TVMMPRCodeCODE] ASC),
    CONSTRAINT [FK_TVMMPRCodes_Ad] FOREIGN KEY ([AdID]) REFERENCES [dbo].[Ad] ([AdID]),
    CONSTRAINT [FK_TVMMPRCodes_MARKETMASTER] FOREIGN KEY ([ApprovedMarketID]) REFERENCES [dbo].[Market] ([MarketID])
);


GO
CREATE TRIGGER [dbo].[TVMMPRCode_AuditTrail] ON [dbo].[TVMMPRCode] 
