CREATE TABLE [dbo].[CreativeDetailStagingTV] (
    ...
    CONSTRAINT [PK_CreativeDetailsTVStaging] PRIMARY KEY CLUSTERED ([CreativeDetailStagingTVID] ASC),
    CONSTRAINT [FK_CreativeDetailTVStg_OccrncDetailsTV] FOREIGN KEY ([OccurrenceID]) REFERENCES [dbo].[OccurrenceDetailTV] ([OccurrenceDetailTVID])
);


GO
CREATE NONCLUSTERED INDEX [idx_MediaFileName]
    ON [dbo].[CreativeDetailStagingTV]([MediaFileName] ASC)
    INCLUDE([CreativeDetailStagingTVID], [CreativeResolution]);


GO
CREATE TRIGGER [dbo].[CreativeDetailStagingTV_AuditTrail] ON [dbo].[CreativeDetailStagingTV] 
...
SET @TableName = 'CreativeDetailStagingTV'
