CREATE TABLE [dbo].[RefAssetRepository] (
    [RefAssetRepositoryID] INT           IDENTITY (1, 1) NOT NULL,
    [MediaStreamID]        INT           NULL,
    [UserGroup]            VARCHAR (150) NULL,
    [AssetSystem]          VARCHAR (150) NULL,
    [CreativeRepository]   VARCHAR (250) NULL,
    CONSTRAINT [PK_RefAssetRepository] PRIMARY KEY CLUSTERED ([RefAssetRepositoryID] ASC)
);


GO
CREATE TRIGGER [dbo].[RefAssetRepository_AuditTrail] ON [dbo].[RefAssetRepository] 
...
SET @TableName = 'RefAssetRepository'
