CREATE TABLE [dbo].[AdProduct] (
    [AdProductID] INT      IDENTITY (1, 1) NOT NULL,
    [AdID]        INT      NOT NULL,
    [ProductID]   INT      NOT NULL,
    [CreateDate]  DATETIME CONSTRAINT [DF_AdProduct_CreateDate] DEFAULT (getdate()) NULL,
    [CreatedBy]   INT      NULL,
    CONSTRAINT [PK_AdProduct] PRIMARY KEY CLUSTERED ([AdProductID] ASC),
    CONSTRAINT [FK_AdProduct_To_Ad] FOREIGN KEY ([AdID]) REFERENCES [dbo].[Ad] ([AdID]) ON DELETE CASCADE,
    CONSTRAINT [FK_AdProduct_To_RefProduct] FOREIGN KEY ([ProductID]) REFERENCES [dbo].[RefProduct] ([RefProductID]) ON DELETE CASCADE
);


GO
CREATE TRIGGER [dbo].[AdProduct_AuditTrail] ON [dbo].[AdProduct] 
