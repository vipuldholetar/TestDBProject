CREATE TABLE [dbo].[MobileCaptureSession] (
    [MobileCaptureSessionID] INT      IDENTITY (1, 1) NOT NULL,
    [MobileJobScheduleID]    INT      NULL,
    [CaptureStartDT]         DATETIME NULL,
    [CaptureEndDT]           DATETIME NULL,
    [MobileTrackingMediaID]  INT      NOT NULL,
    [NodeID]                 INT      NULL,
    [MobileDeviceID]         INT      NULL,
    [CTLegacySeq]            INT      NULL,
    CONSTRAINT [PK_MobileCaptureSession] PRIMARY KEY CLUSTERED ([MobileCaptureSessionID] ASC),
    CONSTRAINT [FK_MobileCaptureSession_MobileDevice] FOREIGN KEY ([MobileDeviceID]) REFERENCES [dbo].[MobileDevice] ([MobileDeviceID]),
    CONSTRAINT [FK_MobileCaptureSession_MobileJobSchedule] FOREIGN KEY ([MobileJobScheduleID]) REFERENCES [dbo].[MobileJobSchedule] ([MobileJobScheduleID]),
    CONSTRAINT [FK_MobileCaptureSession_MobileTrackingMedia] FOREIGN KEY ([MobileTrackingMediaID]) REFERENCES [dbo].[MobileTrackingMedia] ([MobileTrackingMediaID])
);


GO
CREATE NONCLUSTERED INDEX [IX_MobileCaptureSession_CTLegacySeq]
    ON [dbo].[MobileCaptureSession]([CTLegacySeq] ASC);


GO
CREATE NONCLUSTERED INDEX [IX_MobileCaptureSession_CaptureSessionID_CaptureStartDT]
    ON [dbo].[MobileCaptureSession]([MobileCaptureSessionID] ASC, [CaptureStartDT] ASC);


GO
CREATE TRIGGER [dbo].[MobileCaptureSession_AuditTrail] ON [dbo].[MobileCaptureSession] 
...
SET @TableName = 'MobileCaptureSession'
