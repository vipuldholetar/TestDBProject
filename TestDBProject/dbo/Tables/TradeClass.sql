CREATE TABLE [dbo].[TradeClass] (
    [TradeClassID]    INT          IDENTITY (1, 1) NOT FOR REPLICATION NOT NULL,
    [Descrip]         VARCHAR (50) NULL,
    [NoFamilyInd]     TINYINT      CONSTRAINT [DF_TRADECLASS_NoFamilyInd] DEFAULT ((0)) NULL,
    [LimitedEntryInd] TINYINT      NULL,
    [IndPageDT]       TINYINT      NULL,
    CONSTRAINT [PK_TRADECLASS] PRIMARY KEY CLUSTERED ([TradeClassID] ASC)
);


GO
CREATE NONCLUSTERED INDEX [IX_TradeClass_IDDesc]
    ON [dbo].[TradeClass]([TradeClassID] ASC, [Descrip] ASC);


GO
CREATE TRIGGER [dbo].[TradeClass_AuditTrail] ON [dbo].[TradeClass] 
