CREATE TABLE [dbo].[TVEthnicPRCode] (
    [TVEthnicPRCodeID] VARCHAR (200) NOT NULL,
    [EthnicGroupID]    INT           NOT NULL,
    [OriginalPRCode]   VARCHAR (200) NOT NULL,
    [CreatedDT]        DATETIME      NOT NULL,
    CONSTRAINT [PK_TVEthnicPRCodes] PRIMARY KEY CLUSTERED ([TVEthnicPRCodeID] ASC),
    CONSTRAINT [FK_TVEthnicPRCodes_LANGUAGEMASTER] FOREIGN KEY ([EthnicGroupID]) REFERENCES [dbo].[Language] ([LanguageID])
);


GO
CREATE TRIGGER [dbo].[TVEthnicPRCode_AuditTrail] ON [dbo].[TVEthnicPRCode] 
