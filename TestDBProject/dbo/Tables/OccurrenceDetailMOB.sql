CREATE TABLE [dbo].[OccurrenceDetailMOB] (
    ...
    [CreatedDT]             DATETIME     CONSTRAINT [DF_OccurrenceDetailMOB_CreateDTM] DEFAULT (getdate()) NOT NULL,
    [ModifiedDT]            DATETIME     NULL,
    [CreativeSignature]     VARCHAR (50) NOT NULL,
    [OldPatternID]          INT          NULL,
    CONSTRAINT [PK_OccurrenceDetailMOB] PRIMARY KEY CLUSTERED ([OccurrenceDetailMOBID] ASC),
    CONSTRAINT [FK_OccurrenceDetailMOB_CaptureSession] FOREIGN KEY ([CaptureSessionID]) REFERENCES [dbo].[MobileCaptureSession] ([MobileCaptureSessionID]),
    CONSTRAINT [FK_OccurrenceDetailMOB_LandingPage] FOREIGN KEY ([LandingPageID]) REFERENCES [dbo].[LandingPage] ([LandingPageID]),
    CONSTRAINT [FK_OccurrenceDetailMOB_ScrapePage] FOREIGN KEY ([ScrapePageID]) REFERENCES [dbo].[ScrapePage] ([ScrapePageID])
);


GO
CREATE NONCLUSTERED INDEX [IX_OccurrenceDetailMOB_Pattern_ScrapeSession]
    ON [dbo].[OccurrenceDetailMOB]([PatternID] ASC, [CaptureSessionID] ASC);


GO
CREATE TRIGGER [dbo].[OccurrenceDetailMOB_AuditTrail] ON [dbo].[OccurrenceDetailMOB] 
...
SET @TableName = 'OccurrenceDetailMOB'
