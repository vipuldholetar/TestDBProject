CREATE TABLE [dbo].[OccurrenceDetailONV] (
    [OccurrenceDetailONVID] INT          IDENTITY (1, 1) NOT NULL,
    ...
    [CreatedDT]             DATETIME     CONSTRAINT [DF_OccurrenceDetailONV_CreateDTM] DEFAULT (getdate()) NOT NULL,
    [ModifiedDT]            DATETIME     NULL,
    [CreativeSignature]     VARCHAR (50) NOT NULL,
    [CTLegacySeq]           INT          NULL,
    CONSTRAINT [PK_OccurrenceDetailONV] PRIMARY KEY CLUSTERED ([OccurrenceDetailONVID] ASC),
    CONSTRAINT [FK_OccurrenceDetailONV_LandingPage] FOREIGN KEY ([LandingPageID]) REFERENCES [dbo].[LandingPage] ([LandingPageID]),
    CONSTRAINT [FK_OccurrenceDetailONV_ScrapePage] FOREIGN KEY ([ScrapePageID]) REFERENCES [dbo].[ScrapePage] ([ScrapePageID]),
    CONSTRAINT [FK_OccurrenceDetailONV_ScrapeSession] FOREIGN KEY ([ScrapeSessionID]) REFERENCES [dbo].[ScrapeSession] ([ScrapeSessionID])
);


GO
CREATE NONCLUSTERED INDEX [IDX_FK_PatternMasterStagingID]
    ON [dbo].[OccurrenceDetailONV]([PatternStagingID] ASC)
    INCLUDE([ScrapeSessionID], [CreativeSignature]);


GO
CREATE NONCLUSTERED INDEX [IX_OccurrenceDetailONV_Pattern_ScrapeSession]
    ON [dbo].[OccurrenceDetailONV]([PatternID] ASC, [ScrapeSessionID] ASC);


GO
CREATE TRIGGER [dbo].[OccurrenceDetailONV_AuditTrail] ON [dbo].[OccurrenceDetailONV] 
...
SET @TableName = 'OccurrenceDetailONV'
