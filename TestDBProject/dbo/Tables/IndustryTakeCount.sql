CREATE TABLE [dbo].[IndustryTakeCount] (
    [IndustryTakeCountID] INT          IDENTITY (1, 1) NOT NULL,
    ...
    PRIMARY KEY CLUSTERED ([IndustryTakeCountID] ASC),
    CONSTRAINT [FK_IndustryTakeCount_To_Language] FOREIGN KEY ([LanguageID]) REFERENCES [dbo].[Language] ([LanguageID]),
    CONSTRAINT [FK_IndustryTakeCount_To_WWTIndustry] FOREIGN KEY ([WWTIndustryID]) REFERENCES [dbo].[WWTIndustry] ([WWTIndustryID])
);


GO
CREATE TRIGGER [dbo].[IndustryTakeCount_AuditTrail] ON [dbo].[IndustryTakeCount] 
...
SET @TableName = 'IndustryTakeCount'
