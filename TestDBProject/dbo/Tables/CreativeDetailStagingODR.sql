CREATE TABLE [dbo].[CreativeDetailStagingODR] (
    [CreativeDetailStagingODRID] INT           IDENTITY (1, 1) NOT NULL,
    [CreativeStagingID]          INT           NOT NULL,
    [CreativeFileType]           CHAR (10)     NOT NULL,
    [CreativeRepository]         VARCHAR (200) NULL,
    [CreativeAssetName]          VARCHAR (200) NOT NULL,
    [CreativeFileSize]           BIGINT        NOT NULL,
    [CreatedDT]                  DATETIME      DEFAULT (getdate()) NULL,
    [AdFormatId]                 INT           NULL,
    CONSTRAINT [PK_CreativeDetailsODRStaging] PRIMARY KEY CLUSTERED ([CreativeDetailStagingODRID] ASC),
    CONSTRAINT [FK_CreativeDetailStagingODR_To_ODRAdFormatMap ] FOREIGN KEY ([AdFormatId]) REFERENCES [dbo].[ODRAdFormatMap] ([MTFormatID])
);


GO
CREATE UNIQUE NONCLUSTERED INDEX [IX_CreativeDetailStagingODR_CreativeAssetName]
    ON [dbo].[CreativeDetailStagingODR]([CreativeAssetName] ASC);


GO
CREATE NONCLUSTERED INDEX [idx_CreativeStagingID]
    ON [dbo].[CreativeDetailStagingODR]([CreativeStagingID] ASC);


GO
CREATE TRIGGER [dbo].[CreativeDetailStagingODR_AuditTrail] ON [dbo].[CreativeDetailStagingODR] 
...
SET @TableName = 'CreativeDetailStagingODR'
