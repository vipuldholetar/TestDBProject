CREATE TABLE [dbo].[TVDaypartRatesNetwork] (
    [TVDaypartRatesNetworkId] INT          IDENTITY (1, 1) NOT NULL,
    [TVStationId]             INT          NOT NULL,
    [TVDaypartTimeZoneId]     INT          NOT NULL,
    [ProgramSubCatId]         INT          NOT NULL,
    [Rate]                    NUMERIC (18) NOT NULL,
    [EffectiveStartDate]      DATE         NOT NULL,
    [EffectiveEndDate]        DATE         NULL,
    [ModifyDT]                DATETIME     NULL,
    CONSTRAINT [PK_TVDaypartRatesNetwork] PRIMARY KEY CLUSTERED ([TVDaypartRatesNetworkId] ASC),
    CONSTRAINT [FK_TVDaypartRatesNetwork_To_SQADProgramSubCat] FOREIGN KEY ([ProgramSubCatId]) REFERENCES [dbo].[TVSQADProgramSubCat] ([ProgramSubCatId]),
    CONSTRAINT [FK_TVDaypartRatesNetwork_To_TVStation] FOREIGN KEY ([TVStationId]) REFERENCES [dbo].[TVStation] ([TVStationID]),
    CONSTRAINT [FK_TVDaypartsRatesNetwork_To_DaypartTimeZone] FOREIGN KEY ([TVDaypartTimeZoneId]) REFERENCES [dbo].[TVDaypartTimeZone] ([TVDaypartTimeZoneID])
);


GO
CREATE TRIGGER [dbo].[TVDaypartRatesNetwork_AuditTrail] ON [dbo].[TVDaypartRatesNetwork] 
...
SET @TableName = 'TVDaypartRatesNetwork'
