CREATE TABLE [dbo].[WWTIndustry] (
    [WWTIndustryID]             INT           IDENTITY (1, 1) NOT NULL,
    [RefIndustryGroupID]        INT           NOT NULL,
    ...
    PRIMARY KEY CLUSTERED ([WWTIndustryID] ASC),
    CONSTRAINT [FK_WWTIndustry_To_RefClassificationGroup] FOREIGN KEY ([ClassificationGroupId]) REFERENCES [dbo].[RefClassificationGroup] ([RefClassificationGroupID]),
    CONSTRAINT [FK_WWTIndustry_To_RefIndustryGroup] FOREIGN KEY ([RefIndustryGroupID]) REFERENCES [dbo].[RefIndustryGroup] ([RefIndustryGroupID])
);


GO
CREATE NONCLUSTERED INDEX [IX_WWTIndustry_RefIndustryGroup]
    ON [dbo].[WWTIndustry]([RefIndustryGroupID] ASC);


GO
CREATE TRIGGER [dbo].[WWTIndustry_AuditTrail] ON [dbo].[WWTIndustry] 
