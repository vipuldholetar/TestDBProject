CREATE TABLE [dbo].[OccurrenceDetailTV] (
    ...
    [StationID]             INT           NULL,
    CONSTRAINT [PK_OccurrenceDetailsTV] PRIMARY KEY CLUSTERED ([OccurrenceDetailTVID] ASC),
    CONSTRAINT [FK_AdID] FOREIGN KEY ([AdID]) REFERENCES [dbo].[Ad] ([AdID]),
    CONSTRAINT [FK_PatternMasterId] FOREIGN KEY ([PatternID]) REFERENCES [dbo].[Pattern] ([PatternID])
);


GO
CREATE NONCLUSTERED INDEX [IDX_OCCURRENCE_DETAIL_COMB1]
    ON [dbo].[OccurrenceDetailTV]([PatternID] ASC, [AdID] ASC, [PRCODE] ASC)
    INCLUDE([OccurrenceDetailTVID], [TVRecordingScheduleID], [AirDT], [AdLength], [CaptureStationCode]);


GO
CREATE NONCLUSTERED INDEX [IX_OccurrenceDetailTV_Station_AirDate_Deleted]
    ON [dbo].[OccurrenceDetailTV]([StationID] ASC, [AirDT] ASC, [Deleted] ASC);


GO
CREATE TRIGGER [dbo].[OccurrenceDetailTV_AuditTrail] ON [dbo].[OccurrenceDetailTV] 
...
SET @TableName = 'OccurrenceDetailTV'
