CREATE TABLE [dbo].[MTODRGroup] (
    [MTODRGroupID] INT            IDENTITY (1, 1) NOT NULL,
    [GroupDescrip] VARCHAR (1000) NOT NULL,
    [CreatedDT]    DATETIME       DEFAULT (getdate()) NOT NULL,
    [CreatedByID]  INT            NOT NULL,
    [ModifiedDT]   DATETIME       NULL,
    [ModifiedByID] INT            NULL,
    CONSTRAINT [PK_MTODGroups] PRIMARY KEY CLUSTERED ([MTODRGroupID] ASC)
);


GO
CREATE UNIQUE NONCLUSTERED INDEX [IX_MTODRGroup_Descrip]
    ON [dbo].[MTODRGroup]([GroupDescrip] ASC);


GO
CREATE TRIGGER [dbo].[MTODRGroup_AuditTrail] ON [dbo].[MTODRGroup] 
