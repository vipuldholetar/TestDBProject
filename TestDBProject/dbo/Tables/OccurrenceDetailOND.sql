CREATE TABLE [dbo].[OccurrenceDetailOND] (
    [OccurrenceDetailONDID] INT          IDENTITY (1, 1) NOT NULL,
    [PatternStagingID]      INT          NULL,
    [PatternID]             INT          NULL,
    [ScrapeSessionID]       INT          NOT NULL,
    [ScrapePageID]          INT          NOT NULL,
    [LandingPageID]         INT          NULL,
    [AdID]                  INT          NULL,
    [ResultID]              BIGINT       NOT NULL,
    [OccurrenceDT]          DATETIME     NOT NULL,
    [CreatedDT]             DATETIME     CONSTRAINT [DF_OccurrenceDetailOND_CreateDTM] DEFAULT (getdate()) NOT NULL,
    [ModifiedDT]            DATETIME     NULL,
    [CreativeSignature]     VARCHAR (50) NOT NULL,
    [CTLegacySeq]           INT          NULL,
    CONSTRAINT [PK_OccurrenceDetailOND] PRIMARY KEY CLUSTERED ([OccurrenceDetailONDID] ASC),
    CONSTRAINT [FK_OccurrenceDetailOND_LandingPage] FOREIGN KEY ([LandingPageID]) REFERENCES [dbo].[LandingPage] ([LandingPageID]),
    CONSTRAINT [FK_OccurrenceDetailOND_ScrapePage] FOREIGN KEY ([ScrapePageID]) REFERENCES [dbo].[ScrapePage] ([ScrapePageID]),
    CONSTRAINT [FK_OccurrenceDetailOND_ScrapeSession] FOREIGN KEY ([ScrapeSessionID]) REFERENCES [dbo].[ScrapeSession] ([ScrapeSessionID])
);


GO
CREATE NONCLUSTERED INDEX [IDX_FK_PatternMasterStagingID]
    ON [dbo].[OccurrenceDetailOND]([PatternStagingID] ASC)
    INCLUDE([OccurrenceDetailONDID], [ScrapeSessionID], [CreativeSignature]);


GO
CREATE NONCLUSTERED INDEX [IX_OccurrenceDetailOND_Pattern_ScrapeSession]
    ON [dbo].[OccurrenceDetailOND]([PatternID] ASC, [ScrapeSessionID] ASC);


GO

CREATE TRIGGER [dbo].[OccurrenceDetailOND_AuditTrail] ON [dbo].[OccurrenceDetailOND] 
...
SET @TableName = 'OccurrenceDetailOND'
