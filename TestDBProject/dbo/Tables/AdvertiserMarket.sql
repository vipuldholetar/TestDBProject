CREATE TABLE [dbo].[AdvertiserMarket] (
    [AdvertiserID]       INT NOT NULL,
    [MarketID]           INT NULL,
    [AllMarketIndicator] BIT DEFAULT ((0)) NOT NULL,
    CONSTRAINT [FK_AdvertiserMarket_ADVERTISERMASTER] FOREIGN KEY ([AdvertiserID]) REFERENCES [dbo].[Advertiser] ([AdvertiserID]),
    CONSTRAINT [FK_AdvertiserMarket_MARKETMASTER] FOREIGN KEY ([MarketID]) REFERENCES [dbo].[Market] ([MarketID])
);


GO
CREATE TRIGGER [dbo].[AdvertiserMarket_AuditTrail] ON [dbo].[AdvertiserMarket] 
...
SET @TableName = 'AdvertiserMarket'
