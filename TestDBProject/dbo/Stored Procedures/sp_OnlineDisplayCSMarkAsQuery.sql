...
-- Description			: Update Query in Online Display Work Queue  
-- Execution Process	: [dbo].[sp_OnlineDisplayCSMarkAsQuery]
-- Updated By			: 
--===============================================================================================================
CREATE PROCEDURE [dbo].[sp_OnlineDisplayCSMarkAsQuery]
(
...
BEGIN
		...
		BEGIN TRY
				...
				UPDATE  [dbo].[PatternStaging] SET [Query]=@IsQuery,ModifiedDT=Getdate(),ModifiedByID=@UserId WHERE [CreativeStgID]=@CreativeMasterStgid
				...
				SELECT @PatternMasterStagingID=[PatternStagingID] From [dbo].[PatternStaging] WHERE [CreativeStgID]=@CreativeMasterStgid
					...
					VALUES
					(
					@PatternMasterStagingID,NULL,NULL,NULL,NULL,'OND','I&O','PAT',@QueryCategory,@QueryText,@UserId,getdate(),NULL,NULL,NULL,NULL,getdate(),@UserId,@AssignedTo
					) 
		...
		END TRY
		 ...
		 BEGIN CATCH 
			  ...
			  RAISERROR ('[dbo].[sp_OnlineDisplayCSMarkAsQuery]: %d: %s',16,1,@error,@message,@lineNo); 
