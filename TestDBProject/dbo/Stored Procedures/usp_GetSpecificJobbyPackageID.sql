...
-- Query : exec usp_GetSpecificJobbyPackageID 'JP01'  
-- =============================================   
CREATE PROC [dbo].[usp_GetSpecificJobbyPackageID] @jobpackageid VARCHAR(20), @isOnlyActive VARCHAR(20) 
AS 
  BEGIN 
      SET nocount ON 
	  ...
	  select @jobid=[JobCODE] from [JobPackage] where [JobPackageCODE]=@jobpackageid
	  ...
	  BEGIN  
         ...
         SELECT j.id, 
          ...
          WHERE  [JobCODE] = @jobid 
                  ...
                  and [JobPackageCODE]=@jobpackageid  order by [Order]
	  END
      ...
      BEGIN catch 
          ...
          RAISERROR ('usp_GetSpecificJobbyPackageID: %d: %s',16,1,@error,@message,@lineNo); 
      END catch; 
  END;