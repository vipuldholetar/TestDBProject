...
-- Description:  Ingestion of overnight cable file
-- Query : exec sp_NLSInsertONCableFile
-- ============================================= 
CREATE PROC [dbo].[sp_NLSInsertONCableFile]  
AS 
  BEGIN       
      ...
      BEGIN try 
	   ...
	   BEGIN TRANSACTION 
		...
		DECLARE @CableHouseholdFilteredData TABLE
		(
		  [Station] VARCHAR(50),
		  [AirDate] VARCHAR(250),
		...
		DECLARE @CableHouseholdCalcData TABLE
		(
		  [Station] VARCHAR(50),
		  [AirDate] VARCHAR(250),
		  [HalfHrsIdentifier] INT,
		...
		DECLARE @CableHouseholdFinalData TABLE
		(
		  [Station] VARCHAR(50),
		  [AirDate] VARCHAR(250),		
		  [StartTime] VARCHAR(10),
		...
		---DEMOGRAPHIC VARIABLES---
		 ...
		 DECLARE @OvernightCableDemographicFilteredData TABLE
		(
		...
		DECLARE @OvernightCableDistinctDemographicFilteredData TABLE
		(
		  RowId INT IDENTITY (1, 1), 
		  Station VARCHAR(50),
		  AirDate VARCHAR(50),
		...
		DECLARE @OvernightCableDemographicData TABLE
		(
		  RowId INT IDENTITY (1, 1), 
		  Station VARCHAR(50),
		  AirDate VARCHAR(50),
		...
		DECLARE @OvernightCableDemographicCalculatedData TABLE
		(
		  Station VARCHAR(50), 
		  AirDate VARCHAR(50),
		  QuarterHrsId VARCHAR(50),
		...
		INSERT INTO @CableHouseholdFilteredData
		(
		[Station],
		[AirDate],
		[HalfHrsIdentifier],
		...
		CASE WHEN(LEN(StartTime) = 1) THEN
		'000'+Convert( varchar(4),StartTime) 
		WHEN(LEN(StartTime) = 2) THEN
		'00'+Convert( varchar(4),StartTime) 
		WHEN(LEN(StartTime) = 3) THEN
		'0'+ Convert( varchar(4),StartTime)  
		WHEN(LEN(StartTime) = 4) THEN
		Convert( varchar(4),StartTime) END[StartTime],		
		[HHFirst], [HHSecond]
		FROM(	

		SELECT [Station], 
		CASE WHEN(StartTime >= 2400) THEN
		DATEADD(day,1,AirDate) ELSE AirDate END[AirDate], 
		
		[HalfHrsIdentifier], 
		CASE WHEN(StartTime >= 2400) THEN
		(StartTime-2400)ELSE StartTime
		END[StartTime],
		--[StartTime],
		[HHFirst], [HHSecond]
		FROM(	
		SELECT DISTINCT	
		SUBSTRING(NLSRawData,71,6)[Station],
		convert(DATE,'20'+substring((SUBSTRING(NLSRawData,44,7)),2,6))[AirDate], 
		CAST(SUBSTRING(NLSRawData,81,2) AS INT) [HalfHrsIdentifier],
		SUBSTRING(NLSRawData,86,4)[StartTime],		
		SUBSTRING(NLSRawData,260,9)[HHFirst],
		SUBSTRING(NLSRawData,309,9)[HHSecond]
		FROM [NLSNCableRawData]
		WHERE SUBSTRING(NLSRawData,85,1)='H'
		AND SUBSTRING(NLSRawData,1,2)='05'
		) FIL
		)LTR
		)FFF
		
		--
		INSERT INTO @CableHouseholdCalcData
		(
		[Station],
		[AirDate],
		[HalfHrsIdentifier],
		...
		(SELECT TOP 1 StartTime FROM @CableHouseholdFilteredData WHERE Station=CD.Station AND AirDate=CD.AirDate AND HalfHrsIdentifier=CD.HalfHrsIdentifier)[StartTime], 
		...
		from @CableHouseholdFilteredData CD
		--WHERE CD.[FirstQtrHrsDuration]<>0 AND CD.[SecondQtrHrsDuration]<>0
		--AND Convert(int,CD.StartTime)<2400
		group by Station,AirDate,HalfHrsIdentifier, HHFirst, HHSecond
		) FILTER;	

		--

		INSERT INTO @CableHouseholdFinalData
		(
		[Station],
		[AirDate],		
		[StartTime],
		...
		@CableHouseholdCalcData CR,
		@Double CD

		--INSERT FILTERED DEMOGRAPHIC DATA.
		INSERT INTO @OvernightCableDemographicFilteredData
		(Station,
		AirDate,
		QuarterHrsId,
		DemographicGroupId,
		...
		FROM (
		SELECT DATEADD(SECOND,59,DATEVALUE)[DATEVALUE] FROM (
		SELECT  DATEADD(MINUTE,(convert(int,15)),CAST(
									(   SUBSTRING(StartTime, 1, 2) + ':' + 
										SUBSTRING(StartTime, 3, 2)
									) AS DATETIME
								)
		...
		SELECT SUBSTRING(NLSRawData,71,6)[Station],
		convert(DATE,'20'+substring((SUBSTRING(NLSRawData,44,7)),2,6))[AirDate],
        ...
        END						[M65PLUS] 
		FROM [NLSNCableRawData] 
		WHERE SUBSTRING(NLSRawData,1,2)='05'
		AND SUBSTRING(NLSRawData,85,1)='P'
		AND SUBSTRING(NLSRawData,11,1)=0
		AND SUBSTRING(NLSRawData,42,2) = '00'
		AND SUBSTRING(NLSRawData,83,2) != ''
		--AND SUBSTRING(NLSRawData,71,6) = '006111'
		...
		INSERT INTO @OvernightCableDistinctDemographicFilteredData
		(Station,
		AirDate,
		QuarterHrsId,
		DemographicGroupId
		...
		FROM @OvernightCableDemographicFilteredData
		
		--select * from @OvernightCableDemographicFilteredData
        ...
        FROM   @OvernightCableDistinctDemographicFilteredData 
          ...
          WHILE ( @RowCount > 0 ) 
            BEGIN 
                BEGIN try 
					...
					SELECT @Station = Station FROM @OvernightCableDistinctDemographicFilteredData WHERE RowId = @Count
					SELECT @AirDate = AirDate FROM @OvernightCableDistinctDemographicFilteredData WHERE RowId = @Count
					SELECT @QuarterHrsId = QuarterHrsId FROM @OvernightCableDistinctDemographicFilteredData WHERE RowId = @Count
					SELECT @DemographicGroupId = DemographicGroupId FROM @OvernightCableDistinctDemographicFilteredData WHERE RowId = @Count
					
					SELECT @RecordCountForDuration = COUNT(*) FROM @OvernightCableDemographicFilteredData WHERE Station = @Station and AirDate = @AirDate and QuarterHrsId = @QuarterHrsId AND DemographicGroupId = @DemographicGroupId
                 ...
                 + Cast(@RecordCountForDuration AS VARCHAR) ) 
					...
					BEGIN
					UPDATE @OvernightCableDemographicFilteredData SET Duration = '15' WHERE Station = @Station and AirDate = @AirDate and QuarterHrsId = @QuarterHrsId AND DemographicGroupId = @DemographicGroupId
					END
					ELSE
					BEGIN
					...
					INSERT INTO @OvernightCableDemographicData
					(Station,
					AirDate,
					QuarterHrsId,
					DemographicGroupId,
					...
					SELECT @InnerCount = COUNT(*) FROM @OvernightCableDemographicData
					...
					SELECT @StartTime1 = cast(substring(StartTime,1,2)+':'+substring(StartTime,3,2)+':00' as time) FROM @OvernightCableDemographicData WHERE RowId = @InnerStartCount
					SELECT @StartTime2 = cast(substring(StartTime,1,2)+':'+substring(StartTime,3,2)+':00' as time) FROM @OvernightCableDemographicData WHERE RowId = @InnerStartCount + 1
					...
					SELECT @Station1 = Station FROM @OvernightCableDemographicData WHERE RowId = @InnerStartCount
					SELECT @AirDate1 = AirDate FROM @OvernightCableDemographicData WHERE RowId = @InnerStartCount
					SELECT @QuarterHrsId1 = QuarterHrsId FROM @OvernightCableDemographicData WHERE RowId = @InnerStartCount
					SELECT @DemographicGroupId1 = DemographicGroupId FROM @OvernightCableDemographicData WHERE RowId = @InnerStartCount
					SELECT @EndTime1 = EndTime FROM @OvernightCableDemographicData WHERE RowId = @InnerStartCount

					UPDATE @OvernightCableDemographicFilteredData SET Duration = @Duration1 WHERE Station = @Station1 and AirDate = @AirDate1 and QuarterHrsId = @QuarterHrsId1 AND DemographicGroupId = @DemographicGroupId1 AND EndTime = @EndTime1

					SELECT @InnerCount = @InnerCount - 1
					SET @InnerStartCount = @InnerStartCount + 1					
					END
					delete from @OvernightCableDemographicData
					SET @TotalDuration = 0
					END
		...
		INSERT INTO @OvernightCableDemographicCalculatedData
		(Station,
		AirDate,
		QuarterHrsId,
		FC2_5,
		...
		convert(int,round(SUM(convert(float,ISNULL(M65PLUS,0))*Duration)/@QuarterHrDuration,0)) [M65PLUS]
		 FROM 
		@OvernightCableDemographicFilteredData DD
		...
		SELECT (select TVStationId from NLSNMarketStationMap where NLSNStationID = ONH.Station),
		ONH.AirDate,
		...
		FROM @CableHouseholdFinalData ONH
		INNER JOIN @OvernightCableDemographicCalculatedData OND
		ON ONH.Station = OND.Station AND
		ONH.AirDate = OND.AirDate AND
		ONH.QuartedHrsIdentifier = OND.QuarterHrsId
		and exists (select 1 from NLSNMarketStationMap where NLSNStationID = ONH.Station and TVStationID is not null)
			
		--SELECT * FROM @CableHouseholdFilteredData
		--SELECT * FROM @CableHouseholdCalcData
		--SELECT * FROM @CableHouseholdFinalData
		--SELECT * FROM @OvernightCableDemographicFilteredData
		--SELECT * FROM @OvernightCableDemographicCalculatedData
		--SELECT * FROM NLSOvernightNetworkCableRating
      ...
      BEGIN catch 
          ...
          RAISERROR ('sp_NLSInsertONCableFile: %d: %s',16,1,@Error,@Message,@LineNo); 
      END catch; 
  END;