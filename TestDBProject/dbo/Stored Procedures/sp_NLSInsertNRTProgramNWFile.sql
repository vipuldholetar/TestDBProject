...
-- Query : 
/* 
exec sp_NLSInsertNRTProgramNWFile '', 
*/ 
-- ============================================= 
CREATE PROCEDURE [dbo].[sp_NLSInsertNRTProgramNWFile] (@NIELSONINGNWData dbo.NIELSONINGNWDATA readonly) 
AS 
  BEGIN 
    SET nocount ON; 
    BEGIN try 
      ...
      SELECT 1[MARKETCODE], 
             ...
             ( 
                    ...
                    FROM   QuarterHour q
                    WHERE  Cast(CONVERT(DATETIME,Substring(filter.starttime,1,2)+':'+Substring(filter.starttime,3,2)) AS TIME) BETWEEN q.StartTime AND    q.EndTime ) [QUARTERHOURID], 
             Getdate()[CREATEDDATE], 
             1[CREATEDBY] 
      FROM   ( 
      ...
      SELECT acnnetworkcode, 
             ...
             ( 
                    SELECT QuarterHourID 
                    FROM   QuarterHour q
                    WHERE  Cast(CONVERT(DATETIME,Substring(filter.starttime,1,2)+':'+Substring(filter.starttime,3,2)) AS TIME) BETWEEN q.StartTime AND q.EndTime )[QUARTERHOURIDENTIFIER], 
             starttime 
      FROM   ( 
             ...
             CASE 
                    WHEN Substring(value,92,3)='001' THEN 
                           ( 
                                  SELECT QuarterHourID 
                                  FROM   QuarterHour 
                                  WHERE  Cast(CONVERT(DATETIME,Substring((Substring(value,86,4)),1,2)+':'+ Substring((Substring(value,86,4)),3,2)) AS TIME) BETWEEN StartTime AND    EndTime ) 
    ...
    BEGIN catch 
      ...
      SELECT @error = Error_number(), 
             ...
             @lineNo = Error_line() RAISERROR ('sp_NLSInsertNRTProgramNWFile: %d: %s',16,1,@error,@message, @lineNo ); 
