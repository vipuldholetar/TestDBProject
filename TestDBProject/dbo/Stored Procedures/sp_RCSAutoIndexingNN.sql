...
-- Query : exec sp_RCSAutoIndexingNN  
-- =============================================   
CREATE PROCEDURE [dbo].[sp_RCSAutoIndexingNN] (@CreativeMasterStgId INT, 
...
AS 
  BEGIN 
      ...
      BEGIN try          
		  ...
		  Print('LastRunDate : - ' + Cast(@LastRunDate AS VARCHAR) )

          SELECT @MediaStreamStage = MediaStream 
          FROM   [PatternStaging] 
          WHERE  [CreativeStgID] = @CreativeMasterStgId
		 
		  Print('MediaStreamStage : - ' + Cast(@MediaStreamStage AS VARCHAR) )
