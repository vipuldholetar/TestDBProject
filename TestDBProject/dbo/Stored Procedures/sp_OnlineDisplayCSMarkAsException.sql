...
-- Description	: Update Exception for  Online Display Work Queue
-- Execution	: [dbo].[sp_OnlineDisplayCSMarkAsException] 
-- Updated By	: 			   
--=========================================================================
CREATE PROCEDURE [dbo].[sp_OnlineDisplayCSMarkAsException]
(
...
BEGIN
		...
		BEGIN TRY
				...
				BEGIN TRANSACTION 
					SELECT @MediaStream=value from [Configuration] where componentname='Media Stream' and Value='OND'
		...
		END TRY
		 ...
		 BEGIN CATCH 
			  ...
			  RAISERROR ('[dbo].[sp_OnlineDisplayCSMarkAsException]: %d: %s',16,1,@error,@message,@lineNo); 
