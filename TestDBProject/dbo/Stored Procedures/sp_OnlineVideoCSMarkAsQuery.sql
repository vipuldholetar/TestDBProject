...
-- Description			: Update Query in Online Video Work Queue  
-- Execution Process	: [dbo].[sp_OnlineVideoCSMarkAsQuery]
-- Updated By			: 
--===============================================================================================================
CREATE PROCEDURE [dbo].[sp_OnlineVideoCSMarkAsQuery]
(
...
BEGIN
		...
		BEGIN TRY
				...
				SELECT @PatternMasterStagingID=[PatternStagingID] From [dbo].[PatternStaging] WHERE [CreativeStgID]=@CreativeMasterStgid
					...
					VALUES
					(
					@PatternMasterStagingID,NULL,NULL,NULL,NULL,'ONV','I&O','PAT',@QueryCategory,@QueryText,@UserId,getdate(),NULL,NULL,NULL,NULL,getdate(),@UserId,@AssignedTo
					) 
		...
		END TRY
		 ...
		 BEGIN CATCH 
			  ...
			  RAISERROR ('[dbo].[sp_OnlineVideoCSMarkAsQuery]: %d: %s',16,1,@error,@message,@lineNo); 
