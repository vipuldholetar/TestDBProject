...
-- Query : exec sp_TVIngestionForAdAndOccrncData_Old
-- =============================================    
CREATE PROCEDURE [dbo].[sp_TVIngestionForAdAndOccrncData_Old]
AS 
  BEGIN 
      --SET nocount ON;
      BEGIN try 
	  ...
	  --Table variable to hold TV Occurrence data.
		...
		BEGIN
			...
			SELECT @RawOccrncAirDate = [AirDateTime] FROM @RawOccrncTVPlaylist WHERE RowId = @RowCount
			--Fetch PatternCode from playlist file
			SELECT @PatternCode = PatternCode FROM @RawOccrncTVPlaylist WHERE RowId = @RowCount
			--Fetch capturestationcode from playlist file
			SELECT @RawOccrncCaptureStationCode = [CaptureStationCode] FROM @RawOccrncTVPlaylist WHERE RowId = @RowCount
			--Fetch capture time from playlist file
			SELECT @CaptureTime = CaptureTime FROM @RawOccrncTVPlaylist WHERE RowId = @RowCount
			...
			@FinalPRCode,
			@RawOccrncAirDate,
			(SELECT CaptureTime FROM @RawOccrncTVPlaylist WHERE RowId = @RowCount),
			(SELECT [Length] FROM @RawOccrncTVPlaylist WHERE RowId = @RowCount),
			(SELECT CaptureStationCode FROM @RawOccrncTVPlaylist WHERE RowId = @RowCount),
			(SELECT InputFileName FROM @RawOccrncTVPlaylist WHERE RowId = @RowCount),
			(SELECT AirDateTime FROM @RawOccrncTVPlaylist WHERE RowId = @RowCount),
			GETDATE()
			)
			END
      ...
      BEGIN catch 
          ...
          RAISERROR ('sp_TVIngestionForAdAndOccrncData_Old: %d: %s',16,1,@Error,@Message,@LineNo); 
