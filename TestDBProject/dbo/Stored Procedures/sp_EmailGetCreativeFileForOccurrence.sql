...
CREATE PROCEDURE [dbo].[sp_EmailGetCreativeFileForOccurrence] 
(
...
BEGIN
		BEGIN TRY
				...
				SELECT @PatternMasterId=[PatternID] FROM [OccurrenceDetailEM] WHERE [OccurrenceDetailEMID]=@OccurrenceID
					...
					--FROM  [dbo].[CreativeDetailEM] WHERE [dbo].[CreativeDetailEM].[CreativeMasterID]= @CreativeMasterId
   ...
   inner Join [OccurrenceDetailEM] on [OccurrenceDetailEM].OccurrenceDetailEMID = [CreativeStaging].OccurrenceID  where OccurrenceDetailEMID =@OccurrenceID
		...
		BEGIN CATCH
					...
					RAISERROR ('[dbo].[sp_EmailGetCreativeFileForOccurrence] : %d: %s',16,1,@error,@message,@lineNo); 
		END CATCH
END