...
CREATE PROCEDURE [dbo].[sp_UniversalSearch_MOD] 
(
 @XMLDOC AS XML
) 
AS 
  ...
  BEGIN 
      ...
      BEGIN TRY 
				  ...
				  SET @SelectStmnt=' SELECT us.AdID,OriginalAdID,OccurrenceID,Advertiser, MediaStream,MediaStreamId,Visual,LastRunDate,
					...
					IF( @IncludeArchive = 'true' OR @IncludeArchive = 'True' )  --------------IncludeArchive Checked---------------------------------------------------- 
						  BEGIN 
							  ...
							  SET @SelectStmnt=' SELECT TOP '+ CONVERT(VARCHAR, @IncludeArchiveRecordValue) + ' us.AdID,OriginalAdID,OccurrenceID,Advertiser, MediaStream,MediaStreamId,Visual,
					...
					ELSE IF( @ClearMax = 'true' OR @ClearMax = 'True' ) ----------------------Clear Max Checked------------------------------------------------- 
						  BEGIN 
							  SET @SelectStmnt=' SELECT us.AdID,OriginalAdID,OccurrenceID,Advertiser, MediaStream,MediaStreamId,Visual,
							  LastRunDate,CreateDate,LeadAudio,Len AS Length,case  when originaladid is null then ''N'' else ''R'' end   AS RecutType,p.ProductName AS Product,LeadText,RecutDetail,Language,Description,TaglineID, NoTakeAdReason,us.AdvertiserID  ' 
						  END 
					ELSE 
						  BEGIN 
							  SET @SelectStmnt=' SELECT TOP '+ CONVERT(VARCHAR, @ClearMaxRecordValue)+ '  us.AdID,OriginalAdID,OccurrenceID,Advertiser, MediaStream,MediaStreamId,Visual,
							  LastRunDate,CreateDate,LeadAudio,Len AS Length,case  when originaladid is null then ''N'' else ''R'' end   AS RecutType,p.ProductName AS Product,LeadText,RecutDetail,Language,Description,TaglineID,NoTakeAdReason,us.AdvertiserID  ' 
						  END 
