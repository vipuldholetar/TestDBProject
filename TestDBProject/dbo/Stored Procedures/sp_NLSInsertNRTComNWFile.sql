...
-- Description: Process Ingestion for Network Commercial national weekly file.  
-- Query :   
/*  
exec sp_NLSInsertNRTComNWFile '',  

*/ 
-- =============================================   
CREATE PROCEDURE [dbo].[sp_NLSInsertNRTComNWFile] (@NIELSONINGNWData dbo.NIELSONINGNWData 
readonly) 
AS 
  BEGIN 
     -- SET nocount ON; 
...
SELECT acnprogramcode, 
       ...
       airdate, 
       CASE 
         WHEN QuarterHourID = 1 THEN qh.starttime 
         ELSE (SELECT Replace(CONVERT(VARCHAR(6), datevalue, 108), ':', '') 
               FROM   (SELECT Dateadd(minute, ( CONVERT(INT, ( ( 
                                      ...
                                      (( 
                                              Substring(main.starttime, 1, 2) + ':' 
                                              + 
                              Substring(main.starttime, 3, 2) ) AS DATETIME)) AS 
                              DATEVALUE) 
                      Filter) 
       END            [STARTTIME], 
...
[RECORDSCOUNT] 
		...
		--AND rtrim(Substring(value, 15, 6))='ETV'
       ...
       CROSS JOIN QuarterHour QH 
WHERE  QH.QuarterHourID <= recordscount 
...
ORDER  BY HR.acnprogramcode, 
      ...
      BEGIN catch 
          ...
          RAISERROR ('sp_NLSInsertNRTComNWFile: %d: %s',16,1,@error,@message, 
                     @lineNo 
          ); 
