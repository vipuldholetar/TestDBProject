...
-- Execution: [sp_InsertAllCRDataFromStagingToMainTbl_Test] 29712222 , 7055
-- =============================================
CREATE PROCEDURE [dbo].[sp_InsertAllCRDataFromStagingToMainTbl_Test] 
...
BEGIN
	...
	BEGIN TRY
		...
		ELSE	--/// For previously cropped Ad select * from CreativeforCropStaging
				...
				--select * from CropCategoryGroup where FK_CropID=3
								...
								WHERE [CompositeCropStagingID] = @CompositeCropStagingID
								AND [CompositeCropID] IS NOT NULL 
	...
	BEGIN CATCH 
					  ...
					  RAISERROR ('[sp_InsertAllCRDataFromStagingToMainTbl_Test]: %d: %s',16,1,@error,@message,@lineNo); 
