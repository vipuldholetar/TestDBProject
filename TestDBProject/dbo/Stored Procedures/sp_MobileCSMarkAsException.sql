...
-- Description	: Update Exception for  Mobile Work Queue
-- Execution	: [dbo].[sp_MobileCSMarkAsException] 
-- Updated By	: 			   
--=========================================================================
CREATE PROCEDURE [dbo].[sp_MobileCSMarkAsException]
(
...
BEGIN
		...
		BEGIN TRY
				...
				BEGIN TRANSACTION 
					SELECT @MediaStream=value from [Configuration] where componentname='Media Stream' and Value='MOB'
		...
		END TRY
		 ...
		 BEGIN CATCH 
			  ...
			  RAISERROR ('[dbo].[sp_MobileCSMarkAsException]: %d: %s',16,1,@error,@message,@lineNo); 
