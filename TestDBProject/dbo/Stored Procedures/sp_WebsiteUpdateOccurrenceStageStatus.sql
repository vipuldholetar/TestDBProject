...
-- Execute      : [dbo].[sp_UpdateWebsiteOccurrenceStageStatus] 
-- Description	:  Update Occurrence Status  
-- Updated By	: 
--======================================================================

CREATE PROCEDURE [dbo].[sp_WebsiteUpdateOccurrenceStageStatus] 
(
  ...
  BEGIN 
      ...
      BEGIN TRY 
		  ...
		  --Get Status from OccurrenceTable for occurrenceid
			  ...
			  FROM   [OccurrenceDetailWEB] WHERE  [OccurrenceDetailWEBID] = @OccurrenceID 
          ...
          IF @stage = 2  --Update Status for Occurrencetable 
            BEGIN 
                IF @ScanReqdStageStatusID <> @WaitingStatusID  AND @QCReqdStageStatusID <> @WaitingStatusID AND @RouteReqdStageStatusID <> @WaitingStatusID 
                  BEGIN 
                      UPDATE [OccurrenceDetailWEB]  SET    MapStatusID = @CompleteStatusID,IndexStatusID = @CompleteStatusID,OccurrenceStatusID = @CompleteStatusID 
          ...
          IF @stage =3  --Update Status
            BEGIN 
                IF @MapReqdStageStatusID <> @WaitingStatusID AND @QCReqdStageStatusID <> @WaitingStatusID AND @RouteReqdStageStatusID <> @WaitingStatusID 
                  BEGIN 
                      UPDATE [OccurrenceDetailWEB] SET    ScanStatusID = @CompleteStatusID,OccurrenceStatusID = @CompleteStatusID WHERE  [OccurrenceDetailWEBID] = @occurrenceid  
                  END 
                ELSE 
                  BEGIN				  
                      UPDATE [OccurrenceDetailWEB] SET    ScanStatusID = @CompleteStatusID WHERE  [OccurrenceDetailWEBID] = @occurrenceid 
                  END 
            END 
          ...
          IF @stage =4  --Update Status
            BEGIN 
                IF @MapReqdStageStatusID <> @WaitingStatusID AND @ScanReqdStageStatusID <> @WaitingStatusID AND @RouteReqdStageStatusID <> @WaitingStatusID
                  BEGIN 
					 UPDATE [OccurrenceDetailWEB] SET    QCStatusID = @CompleteStatusID,OccurrenceStatusID = @CompleteStatusID WHERE  [OccurrenceDetailWEBID] = @occurrenceid 
                  END 
                ELSE 
                  BEGIN 
      ...
      BEGIN CATCH 
				...
				RAISERROR ('[sp_UpdateWebsiteOccurrenceStageStatus]: %d: %s',16,1,@error,@message,@lineNo); 
