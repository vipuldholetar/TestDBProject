...
-- =============================================    
CREATE PROCEDURE [dbo].[sp_TVIngestionForAdAndOccrncData_Test]
AS 
  BEGIN 
      SET nocount ON;
      BEGIN try 
	  ...
	  --Table variable to hold TV Occurrence data.
		...
		SELECT DISTINCT InputFileName FROM @RawOccrncTVPlaylist
		...
		BEGIN
			...
			[CreateDTM],
			[IngestionDTM] 
			FROM @RawOccrncTVPlaylist WHERE InputFileName = @InputFileName
			...
			SELECT @TotalRows = COUNT(*) FROM @RawTVPlaylist	
				...
				BEGIN
					...
					print ('@InsertCountOccrncDetailsTV: '+ convert(varchar(10),@InsertCountOccrncDetailsTV))
					END					
					END		
					END	
		...
		BEGIN
			SET @TotalTVPatternRows = @TotalTVPatternRows - 1
				...
				IF (NOT EXISTS(SELECT * FROM [PatternTVStaging] WHERE [CreativeSignature] = @FK_PRCode) )
				BEGIN
