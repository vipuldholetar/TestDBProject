       ...
       SET NOCOUNT ON;
					...
					BEGIN TRY
							...
							PRINT @Patternmasterid
								...
								--Creating temp table
								declare @tempPageDefinitionData table
								(
								id int identity(1,1),
								...
								DELETE FROM CreativeDetailSOC WHERE CreativeDetailSOC.CreativeMasterID=@Creativemasterid AND CreativeDetailSOC.[CreativeDetailSOCID] Not in
								 (select CreativeDetailID from @tempPageDefinitionData)
								...
								--IF Exists(Select 1 FROM CreativeDetailSOC WHERE CreativeMasterID=@Creativemasterid)
								--BEGIN
									IF Exists(Select 1  FROM CreativeDetailSOC WHERE CreativeDetailSOC.CreativeMasterID=@Creativemasterid and CreativeDetailSOC.[CreativeDetailSOCID]=@CreativeDetailId)
										BEGIN					
											Update CreativeDetailSOC SET [PageTypeID]=@PageTypeId,[SizeID]=@SizeId,[PageStartDT]=@PageStartDate,[PageEndDT]=@PageEndDate,PageName=@PageName,
											PubPageNumber=@PubPageNumber,PageNumber=@PageNumber  Where CreativeDetailSOC.[CreativeDetailSOCID]=@CreativeDetailId
										END
									ELSE
										BEGIN
								...
								SET @TotalPageCount=(Select Count(*) from CreativeDetailSOC  Where Creativemasterid=@CreativeMasterID)
								SET @OccurrencePubPageNumber=(Select PubPageNumber from CreativeDetailSOC  Where Creativemasterid=@CreativeMasterID and pagenumber=1)

								--Updating Page Count and Pub Page Number Details in OccurrenceDetailSOC
								--Update OccurrenceDetailsSOC Set EmailPageNumber=@OccurrencePubPageNumber,PageCount=@TotalPageCount Where PK_Id=@OccurrenceId
					...
					BEGIN CATCH 
								...
								RAISERROR ('[sp_SocialDPFUpdatePageDefinitionData]: %d: %s',16,1,@error,@message,@lineNo); 
					END CATCH                   
